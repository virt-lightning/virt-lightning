...
def up(virt_lightning_yaml, configuration, context, **kwargs):
    ...
    async def deploy():
        ...
        for host in virt_lightning_yaml:
            futures.append(loop.run_in_executor(pool, _start_domain, hv, host, context, configuration))
...
def start(configuration, **kwargs):
    ...
    print((
        '\033[0m\n**** System is online ****\n'
        'You can continue to follow the console output with: vl console {name}')
...
def stop(configuration, **kwargs):
    ...
    if not domain:
        ...
        print("No VM called {name} in {vm_list}".format(name=kwargs['name'], vm_list=vm_list))
        exit(1)
    hv.clean_up(domain)
...
Example:
 ...
 The file is ready to be used by Ansible:
   $ ansible all -m ping -i inventory"""
    ...
    start_parser = action_subparsers.add_parser("start", help="Start a new VM", parents=[parent_parser])
    start_parser.add_argument("distro", help="Name of the distro", type=str, nargs="?")

    stop_parser = action_subparsers.add_parser("stop", help="Stop a VM", parents=[parent_parser])
    stop_parser.add_argument("name", help="Name of the VM", type=str, nargs="?")
